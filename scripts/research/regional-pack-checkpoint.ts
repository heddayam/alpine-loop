...
if (!PACK_ID) throw new Error(usage());
const DEFAULT_SCENARIOS_PATH = `data/regions/${PACK_ID}/scenarios.json`;
const MAXIMUM_REFERENCE_DISTANCE_METERS = 500;
...
const scenarioFileSchema = z.object({
  ...
  scenarios: z.array(z.object({
    ...
    maximumRepeatedTrailPct: z.number().min(0).max(100).optional(),
...
try {
  ...
  for (const scenario of scenarioFile.scenarios) {
    try {
      ...
      if (selected.distanceMeters > MAXIMUM_REFERENCE_DISTANCE_METERS) {
        throw new Error(
          `Nearest eligible portal is ${selected.distanceMeters.toFixed(1)}m from ${scenario.referencePoint.name}; `
          + `maximum is ${MAXIMUM_REFERENCE_DISTANCE_METERS}m`,
        );
      }
...
console.log(JSON.stringify({
  formatVersion: 1,
  checkpoint: `regional-pack-${PACK_ID}`,
