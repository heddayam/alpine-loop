...
- [ ] Gate 4 — hardening, accessibility, deterministic tests, documentation
  - Evidence: in progress. Desktop UI hardening now uses a fixed, document-scroll-free 44 px shell with 268 px planning and 308 px results rails, a 704 px map at 1280×720, independent rail collapse to a full-width map, compact table-style constraints, selected-only route details, a collapsed map key, and non-overlapping toolbar/status/zoom controls. `npm run verify` passes 181 offline tests across 33 files plus the production build; `npm run test:browser` passes all five Chromium flows, including exact/near-miss honesty, keyboard route selection, 1280×720 rail geometry, document-scroll containment, and full-map collapse. A live in-app-browser check against the installed Santa Cruz pack generated 10 exact routes, kept document scroll at zero, and preserved the 704 px map with both rails open. Fresh-clone prerequisites and the remaining release-readiness audit are still outstanding, so Gate 4 remains unchecked.
