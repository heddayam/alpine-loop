    ...
    <main className="app-frame">
      ...
      <div className={["workspace", hasResultsPanel ? "with-results" : "", desktopBuilderVisible ? "" : "without-builder", hasResultsPanel && !desktopResultsVisible ? "without-results" : ""].filter(Boolean).join(" ")}>
        ...
        <aside className={["builder-panel", mobilePanel === "builder" ? "" : "mobile-panel-hidden", desktopBuilderVisible ? "" : "desktop-panel-hidden"].filter(Boolean).join(" ")} aria-labelledby="builder-title">
          ...
          <section className="builder-section policy-section" aria-labelledby="policy-title"><h3 id="policy-title">Access policy & results</h3><label className="switch-row"><span><strong>Include uncertain access</strong><small>On by default. May include trails without confirmed public access.</small></span><input type="checkbox" role="switch" checked={values.includeUncertainAccess} onChange={(event) => { setValues((current) => ({ ...current, includeUncertainAccess: event.currentTarget.checked })); invalidateResults(); }} /></label><div className="count-field"><label htmlFor="route-count">Number of routes</label><input id="route-count" type="number" min="1" max="20" value={values.limit} onChange={(event) => setValues((current) => ({ ...current, limit: event.currentTarget.value }))} /><small>1 to 20 alternatives</small></div></section>
        ...
        <HikeMap mode={mode} drawBounds={drawnDraft.bounds} drawEnabled={mode === "drawn-area"} filterGeometry={filterGeometry} refinementGeometry={refinementGeometry} packCoverage={pack.coverage} suggestedBounds={pack.suggestedBounds} display={pack.display} trailNetwork={trailNetwork} accessPoints={accessPoints} selectedAccessPointId={selectedAccessPointId} routes={generatedRoutes} selectedRouteId={selectedRouteId} onBoundsChange={onBoundsChange} onAccessPointSelect={(id) => { setSelectedAccessPointId(id); invalidateResults(); }} onRouteSelect={setSelectedRouteId} />
