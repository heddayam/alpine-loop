...
function represented(query:StatementSync,point:Coordinate,radius:number):boolean {
...
function buildOsmIndex(osm:CoverageSourceStore,coverage:AreaGeometry):void {
  const db=osm.db;
...
export async function auditOfficialTrailReferences(options:OfficialReferenceOptions):Promise<OfficialReferenceAudit> {
  ...
  buildOsmIndex(options.osm,options.sourceEnvelope?rectangle(options.sourceEnvelope):options.coverage);
    ...
    for(const feature of features){
      ...
      for(let segment=1;segment<feature.coordinates.length;segment++){
        ...
        for(let index=0;index<samples;index++){
          ...
          if(!represented(near,point,radius))unrepresentedLengthM+=weight;
        }
      }
