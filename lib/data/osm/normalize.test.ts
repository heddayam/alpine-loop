...
describe("OSM hiking topology normalization", () => {
  ...
  it("classifies ways deterministically from road, pedestrian and motor-vehicle context", () => {
    ...
    expect(classifyOsmWay({ highway: "service", foot: "designated" })).toBe("service-road");
    ...
    expect(classifyOsmWay({ highway: "service", service: "access_aisle" })).toBe("sidewalk");
    expect(classifyOsmWay({ highway: "track" })).toBe("trail");
    expect(classifyOsmWay({ highway: "track", access: "private" })).toBe("trail");
    expect(classifyOsmWay({ highway: "track", motor_vehicle: "yes" })).toBe("service-road");
    expect(classifyOsmWay({ highway: "track", vehicle: "designated" })).toBe("service-road");
    expect(classifyOsmWay({ highway: "track", access: "yes" })).toBe("service-road");
