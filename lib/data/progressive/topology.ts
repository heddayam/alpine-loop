...
function scc(db: DatabaseSync): void {
  let finish = 0;
...
function seedCycles(db: DatabaseSync): void {
  let time = 0;
...
function distances(db: DatabaseSync): void {
...
function connectorHash(db: DatabaseSync, start: number, portal: number): string {
...
export function writeProgressiveTopology(db: DatabaseSync, manifest: PackManifest): { hash: string; profiles: Array<{ profile: "known" | "inclusive"; hash: string; feasible: number; physical: number }> } {
  ...
  for (const profile of ["known", "inclusive"] as const) {
    ...
    for (const access of rows(db, `SELECT a.id AS id,n.node_key AS k,w.scc,w.dist,w.portal FROM access_points a JOIN nodes n ON n.id=a.node_id JOIN work_nodes w ON w.k=n.node_key ORDER BY a.id`)) {
      ...
      const record = {
        ...
        connectorKey: canReachCycle ? connectorHash(db, key, portal!) : null,
