...
function completedJob(request: SearchIntent, routeCount: number): RouteJobV2 {
  return {
    version: 2, id: JOB_ID, status: "completed", request,
    area: savedRoutes(request).area,
    progress: { eligibleAccessPointCount: 2, processedAccessPointCount: 2, exactRouteCount: routeCount, nearMissRouteCount: 0, truncatedAccessPointCount: 0, elapsedMs: 1200 },
...
export async function launchAndOpenResults(page: Page): Promise<void> {
  await page.getByRole("button", { name: "Full search", exact: true }).click();
